...
//Tempo cadastrado para o horario atual
...
//Função responsavel por pegar os dados de produtos e exibilos no modal de cadastro
if ($acao == 'exibirDados') {
...
//Requisição de todos os produtos cadastrados
    ...
    $produtosVerificados = [];
//Estrutura que indentifica quais produtos já existe no estoque da escola, para que nao sejam exibidos novamente e evitando
...
//Submissão ao banco de dados do cadastro do produto selecionado
    ...
    $insert = "insert into Estoque values (default, '$idEscola', '$idProduto', NULL, $quantidade, $quantidade, '$data', " . $_SESSION['id'] . ", 'Nut', 'Adicionado', 1)";
    $query = mysqli_query($conexao, $insert);
}

//Coleta dos dados de produto e quantidade de estoque para exibir nos modais de alocação/Redução
...
//Submissão ao banco de dados da alocação em quantidade do produto selecionado 
    ...
    $insert = "insert into Estoque values(default, $idEscola, $idProduto, NULL, $quantidade, $quantidadeRed, '$data', " . $_SESSION['id'] . ", 'Nut', 'Adicionado', 1)";
...
//Submissão ao banco de dados da Redução em quantidade do produto selecionado 
    ...
    $insert = "insert into Estoque values(default, $idEscola, $idProduto, NULL, $quantidade, $quantidadeRed, '$data', " . $_SESSION['id'] . ", 'Nut', 'Retirado', 1)";
    $queryInsert = mysqli_query($conexao, $insert);
}
...
//Atualização do novo estoque da escola base.
        $insertNewEstBase = "insert into Estoque values(default, $idEscola, $idProduto, $idEstoqueAlvo, $quantidadeEstoque, $quantidade, '$data', " . $_SESSION['id'] . ", 'Nut', 'TransfRed', 1)";
...
//Criação do novo estoque da escola alvo
        $insertNewEstoque = "insert into Estoque values(default, $idEscolaAlvo, $idProduto, $idNewEstBase, $quantidadeEstoqueAlvo, $quantidade, '$data', " . $_SESSION['id'] . ", 'Nut', 'TransfAdd', 1)";
        mysqli_query($conexao, $insertNewEstoque);
    } else {
        $insertNewEstBase = "insert into Estoque values(default, $idEscola, $idProduto, NULL, $quantidadeEstoque, $quantidade, '$data', " . $_SESSION['id'] . ", 'Nut', 'TransfRed', 1)";
        $queryInsertNewEstBase = mysqli_query($conexao, $insertNewEstBase);
        ...
        $insertNewEstAlvo = "insert into Estoque values (default, $idEscolaAlvo, $idProduto, $idNewEstBase, $quantidade, $quantidade, '$data', " . $_SESSION['id'] . ", 'Nut', 'TransfAdd', 1)";
